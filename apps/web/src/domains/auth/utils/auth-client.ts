...
export class AuthClient {
  ...
  private async refreshSession({
    tokenSet: { refreshToken: refreshTokenValue },
    deviceId,
    ...prev
  }: SessionData): Promise<SessionData | null> {
    try {
      // NUGUDI API: refreshToken은 헤더로 전달 (Authorization, X-Device-ID)
      ...
      // NUGUDI API 응답 구조: response.data.data (not response.data.result)
      if (
        response.status === 200 &&
